...
subtest "Doesn't create for invalid email addresses" => sub {
    throws_ok { Value::EmailAddressCommon->new(); } qr/^Value::EmailAddressCommon/, "no create undef email address";
    throws_ok { Value::EmailAddressCommon->new( '' ); } qr/^Value::EmailAddressCommon/, "no create empty email address";
};

subtest "Create correct object for simple email address" => sub {
    my $domain = Value::EmailAddressCommon->new( 'gwadej@cpan.org' );
    isa_ok( $domain, 'Value::EmailAddressCommon' );
...
{
    throws_ok { Value::EmailAddressCommon->new( '"gwade@j"@cpan.org' ) }
        qr/^Value::EmailAddressCommon/,
...
subtest "Create fine with new_canonical and simple address" => sub {
    my $domain = Value::EmailAddressCommon->new_canonical( 'gwadej@cpan.org' );
    isa_ok( $domain, 'Value::EmailAddressCommon' );
    is( $domain->value, 'gwadej@cpan.org', "EmailAddressCommon matches input" );
};

{
    throws_ok { Value::EmailAddressCommon->new_canonical( '"g@wadej"@cpan.org' ); }
        qr/^Value::EmailAddressCommon/,
